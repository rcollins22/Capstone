...
import url from '../../url';
import PortfolioDonut from '../PortfolioView/PortfolioDonut';
import NewPerformance from '../../../src/components/NewPerformance';
...
const NewLeaderDash = () => {
  ...
  useEffect(() => {
    ...
    if (portfolio && portfolio != 'NewLeaderDash') {
      getSpecificPortfolioHistory(portfolio);
    }
    if (portfolio && portfolio != 'NewLeaderDash') {
  ...
  const loadTodaysChange = () => {
    var currUid = localStorage.getItem('id'); //exemplar call to local storage
    axios
      .get(`${url}/users/overall-performance/${currUid}?days=2`)
      .then(res => {
  ...
  const getSpecificPortfolioHistory = portId => {
    var currUid = localStorage.getItem('id');
    axios
      .get(`${url}/portfolios/history/${portId}?days=20`)
      .then(res => {
  ...
  const loadPortfolio = portId => {
    axios
      .get(`${url}/portfolios/chart/${portId}`)
      .then(res => {
        console.log(
  ...
  let sendChart =
    localStorage.getItem('onPortfolio') == 'NewLeaderDash' ||
    !localStorage.getItem('onPortfolio')
      ? chartData
      : chart;
  let sendNames =
    localStorage.getItem('onPortfolio') == 'NewLeaderDash' ||
  ...
  return (
    <Page className={classes.root} title="NewLeaderDash">
      <Container maxWidth={false}>
        <Grid container spacing={3}>
          <Grid item lg={3} sm={6} xl={3} xs={12}>
            <TodaysChange chg="-0.09" />
          </Grid>
          <Grid item lg={3} sm={6} xl={3} xs={12}>
            <TotalFollowers f="Following" amt="David Mitchell" />
          </Grid>
          <Grid item lg={3} sm={6} xl={3} xs={12}>
            <TodaysMoney money="--" show="-$9.24" />
          </Grid>
          <Grid item lg={3} sm={6} xl={3} xs={12}>
            <TotalBalance balance='9990.76' />
          </Grid>
          <Grid item lg={8} md={12} xl={9} xs={12}>
            <NewPerformance chartData={sendChart} />
          </Grid>
